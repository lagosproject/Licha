...
def run_cmd(cmd):
    print(f"  → {cmd}")
    # No shell: split the command into argv so interpolated values (paths, locales) can never
    # be interpreted as shell metacharacters. Accepts a pre-tokenized list as-is.
...
def screencap(path):
    ...
    device_path = "/sdcard/licha_screencap.png"
    run_cmd(f"adb shell screencap -p {device_path}")
    run_cmd(f"adb pull {device_path} {path}")
    run_cmd(f"adb shell rm {device_path}")
